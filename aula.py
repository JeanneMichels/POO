...
class TelaInicial(arcade.View):
    ...
    def on_draw(self):
        ...
        if not self.exibindo_objetivos:
            ...
            arcade.draw_text("6. Seja rapido e coma todos os peixes e tartarugas!", 50, 185, arcade.color.WHITE, 16)
    ...
    def on_draw(self):
        ...
        if self.fase == 1:
            ...
            if self.cenario_fase2:
                ...
                arcade.set_background_color(arcade.color.OCEAN_BLUE)
            self.lista_tartarugas.draw()
            
        elif self.fase == 3:
            arcade.set_background_color(arcade.color.DARK_BLUE)
            self.lista_orcas.draw()
            arcade.draw_text(f"Sobreviva: {self.cronometro_fase3:.1f}s", LARGURA / 2, ALTURA - 40, arcade.color.YELLOW, 18, anchor_x="center", bold=True)
